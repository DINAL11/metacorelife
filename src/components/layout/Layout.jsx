  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50/30 pb-20">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200/50">
        <div className="max-w-screen-lg mx-auto px-3 md:px-4 py-3 md:py-4 flex items-center justify-between">
          <button onClick={() => onNavigate('feed')} className="flex items-center gap-1.5 md:gap-2">
            <CompanyLogo size="sm" />
          ...
          <button onClick={() => setShowCart(true)} className="relative p-2">
            <ShoppingCart className="w-5 h-5 md:w-6 md:h-6 text-slate-600 hover:text-cyan-600 transition-colors" />
            {itemCount > 0 && (
